...
Rules that matter:
- Structure is carried by **1px `--color-rule` hairlines** and the **type scale**, never by cards, panels, shadows, or gradients (the Experience rail's fade is the one gradient).
- **The accent** appears only on: section numerals, the hero kicker, links, one bold word in the contact line, and the Experience rail, nodes, and ticks. Never a content fill, never a second hue.
- **`border-radius: 0`** everywhere (the timeline node circle is geometry).
- **Spline Sans Mono** is for data only: dates, indices, labels, stack lists. Never prose or headings. Never `Inter` or a system display face.
...
- All current work is on **`redesign-editorial-dark`**: the Direction C rebuild,
  finish-review fixes, DESIGN.md, the CV content sync, the client-pinned icons, and the
  em-dash cleanup. `93484f8` is the pre-redesign baseline.
