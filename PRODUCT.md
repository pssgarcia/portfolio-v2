...
- **Contact form:** `ContactSection.vue` does a JS `fetch` POST of JSON to `/api/contact`. That endpoint is not built yet, so submissions fail gracefully to the "email me directly" note. No application backend; real delivery (a Vercel function, email, or DB) is a future sprint, not current scope.
